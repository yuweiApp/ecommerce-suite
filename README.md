...
A+ 模块图、多角度细节图、场景穿搭图、卖点图等。给定商品图 URL，脚本走完一条
5 步流水线，最终返回每张图的提示词与生成图片 URL。

> 本仓库是一个 [Hermes](https://github.com/) Agent Skill，`SKILL.md` 是给
> Agent 看的调用规范，`scripts/generate_suite.py` 是唯一的生成入口（一个异步
> HTTP 客户端，把生成服务当黑盒调用）。
...
- **流式进度**：实时打印 5 步流水线进度，最终输出结构化 JSON 结果。
- **省额度模式**：`--no-generate` 只产出提示词、不真正出图。

## 处理流程（5 步流水线）

| 步骤 | 说明 |
|---|---|
| ① 卖点分析 | 分析商品图与关键信息，提炼核心卖点 |
| ② 商品识别 + 抠图 | 识别商品、抠出主体 |
| ③ 推荐套图场景 | 自动推荐（或按自定义）每张图的画面场景 |
| ④ 生成套图提示词 | 为每张图生成图像提示词 |
| ⑤ AIGC 生图 | 调用生图模型产出图片 URL |

## 环境要求

- Python 3.13（脚本以 `uv run` 调用，依赖 `httpx`）
- 一个可访问的电商套图生成服务（默认 `http://192.168.84.54:8077`）
...
优先级：`--api` 参数 > `ECOMMERCE_SUITE_API` 环境变量 > 内置默认值。
...
> 纯净无文字图：`--language ""`。
> 只要提示词、不出图：加 `--no-generate`。

## 参数说明

脚本**不含任何默认内容参数**，需显式传入；未传的参数不进入请求体，交由服务端处理。
...
| `--no-generate` | 可选 | 只产出提示词，不真正生图 |
| `--api` | 可选 | 接口地址（默认取 `ECOMMERCE_SUITE_API`） |

## 输出

运行过程中实时打印 5 步进度、推荐场景与每张图的生成结果，最终在
`===RESULT_JSON===` 标记后输出结构化 JSON：
...
```json
{
  ...
  "productDescription": "...",
  "items": [
    { "name": "...", "aspectRatio": "1:1", "imageUrls": ["..."], "prompt": "..." }
  ]
}
```
...
├── SKILL.md                      # Agent 调用规范
└── scripts/
    └── generate_suite.py         # 生成入口（异步 HTTP 客户端）
```
